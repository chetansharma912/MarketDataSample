	...
	if(window.location.href.includes("zerodha")){
		...
		function sleep(){
			...
			if(cnt>=priceChangeTypeCloseAll.length){
				...
				let stocks = document.querySelectorAll(".instruments .instrument");
				if(!stocks || stocks==null){
					//setTimeout(getStocks,100);
				}
				else{
					stocks.forEach((stock)=>{
						setTimeout(()=>{
							let name = stock.querySelector(".info .nice-name").innerText.trim();
							let ohlc = stock.querySelector(".market-depth .ohlc").innerText.replaceAll(" ","").split("\n");
							let ohlcFilter = [];
							if(ohlc){
								for(let i=0;i<ohlc.length;i++){
									if(i%2!=0){
										if(ohlc[i]!="N/A"){
											// console.log(ohlc[i]);
											ohlcFilter.push(ohlc[i]);
										}
										else{
											alert("OHLC needs to loop again, RELOAD page");
											break;
										}
									}
								}
							...
							maindata.push({
								...
								"high" : ohlcFilter[1],
								"low" : ohlcFilter[2],
								"prevClose" : ohlcFilter[3],
								"LC" : ohlcFilter[8],
								"UC" : ohlcFilter[9]
							});
							
							if(maindata.length>=50){
								//maindata = maindata.slice(0,50);
								//console.log("Ready Morning Market Data",maindata.length,maindata);
								// sendDataToMainPage(maindata);
								fetchData();
							}
...
if(window.location.href.includes("data")){
		chrome.runtime.onMessage.addListener(function(request,sender,sendResponse){
			if(request.message=="showdata"){
				populateDataOnPage(request.data, request.maindata);
			}
...
function fetchIndices(){
	...
	chrome.runtime.sendMessage({"message":"indexes","data":texts,"maindata":maindata},(response)=>{
...
function populateDataOnPage(data,maindata){
	// console.log(data);
	
	//console.log(maindata); // main morning data is fetched here
	...
	indexes.forEach((item)=>{
		...
		if(item.form=="index"){
			indexesData +=` <tr style="font-weight: 700;background-color:${color};"><td>${item.index}</td><td>${item.value}</td><td>${item.percent}%</td><td>${item.absoluteprice}</td><tr> `;
		}
	...
	stocks.forEach((item)=>{
		...
		if(item.form=="stock"){
			...
			stocksData +=` <div style="font-weight: 700;background-color:${color};" class="text-center card col-md-1">${item.index}<br>${item.value}<br>${item.absoluteprice} (${item.percent}%)<br>${item.buy} - ${item.sell}<br>${item.volume}</div> `;
			buyAll += item.buy;
			sellAll += item.sell;
	...
	sorted.forEach((item)=>{
		...
		advancedDeclineData +=` <div style="font-weight: 700;background-color:${color};" class="text-center card col-md-1">
									...
									<span class="${sp}">Pendind Orders : ${item.buy-item.sell}</span>
									<span class="${sp}">Volume : ${item.volume}</span>
								</div> `;
	});
	...
	document.querySelector("#advanced").innerHTML = advanced.length+"<br>"+buyAll;
	document.querySelector("#decline").innerHTML = decline.length+"<br>"+sellAll;
	countTime++;
	if(countTime>=10){
	...
	document.querySelector("#silent").innerHTML = silent.length+"<br>Pendind Orders : "+(buyAll-sellAll)+"<br>Voulme :"+volume+"<br>Change In Volume : "+changeInVolume+" ("+countTime+")";
